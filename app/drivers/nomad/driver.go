...
func (p *config) Create(ctx context.Context, opts *types.InstanceCreateOpts) (*types.Instance, error) { //nolint:gocyclo,funlen
	...
	if p.noop {
		...
		initJob, initJobID, initTaskGroup, err = p.virtualizer.GetInitJob(vm, id, p.userData, p.machinePassword, vmImageConfig, liteEngineHostPort, resource, opts, gitspacesPortMappings)
		if err != nil {
			defer p.deregisterJob(logr, resourceJobID, false) //nolint:errcheck
			return nil, err
		}
