...
func (p *config) Create(ctx context.Context, opts *types.InstanceCreateOpts) (*types.Instance, error) {
	...
	if err != nil {
		defer p.deregisterJob(logr, resourceJobID, false) //nolint:errcheck
		return nil, fmt.Errorf("scheduler: could not register job, err: %w ip: %s, init_job_id: %s, vm: %s", err, ip, initJobID, vm)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("scheduler: could not poll for init job status, failed with error: %s on ip: %s, init_job_id: %s, vm: %s", err, ip, initJobID, vm)
	}
	...
	if err != nil {
		defer p.Destroy(context.Background(), []*types.Instance{instance}) //nolint:errcheck
		return nil, fmt.Errorf("scheduler: init job failed with error: %s on ip: %s, init_job_id: %s, vm: %s", err, ip, initJobID, vm)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("scheduler: could not query resource job, err: %w, init_job_id: %s, vm: %s", err, initJobID, vm)
	}
	if job == nil || isTerminal(job) {
		defer p.Destroy(context.Background(), []*types.Instance{instance}) //nolint:errcheck
		return nil, fmt.Errorf("scheduler: resource job reached unexpected terminal status, removing VM, init_job_id: %s, vm: %s", initJobID, vm)
	}
