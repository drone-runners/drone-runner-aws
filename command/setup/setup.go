...
func (c *setupCommand) run(*kingpin.ParseContext) error { //nolint
	...
	// provision
	instance, _, provisionErr := poolManager.Provision(ctx, testPoolName, runnerName, "drone", "", nil, nil, nil, nil, "", "", false, nil, -1, false, nil)
	if provisionErr != nil {
	...
	if startErr != nil {
		cleanErr := poolManager.Destroy(ctx, testPoolName, instance.ID, instance, nil, nil)
		consoleLogs, consoleErr := poolManager.InstanceLogs(ctx, testPoolName, instance.ID)
		logrus.Infof("setup: instance logs for %s: %s", instance.ID, consoleLogs)
	...
	destroyErr := poolManager.Destroy(ctx, testPoolName, instance.ID, instance, nil, nil)
	return destroyErr
}
