...
func (c *setupCommand) run(*kingpin.ParseContext) error { //nolint
	...
	store, _, _, _, _, err := database.ProvideStore(ctx, database.SingleInstance, "", false, "") //nolint:dogsled
	if err != nil {
	...
	pools, processErr := poolfile.ProcessPool(configPool, runnerName, env.Passwords(), nil)
	if processErr != nil {
	...
	// provision
	instance, _, _, _, provisionErr := poolManager.Provision(ctx, testPoolName, runnerName, "drone", nil, nil, nil, nil, nil, -1, false, nil, false) //nolint:dogsled
	if provisionErr != nil {
