...
func handleDestroy(ctx context.Context, r *VMCleanupRequest, s store.StageOwnerStore, crs store.CapacityReservationStore, enableMock bool, mockTimeout int,
	...
	if err != nil {
		...
		if resp != nil && resp.OSStats != nil {
			...
			metrics.CPUPercentile.WithLabelValues(poolID, inst.OS, inst.Arch, string(inst.Provider), strconv.FormatBool(poolManager.IsDistributed()), inst.VariantID, metric.ConvertBool(inst.GPU)).Observe(resp.OSStats.MaxCPUUsagePct)
			metrics.MemoryPercentile.WithLabelValues(poolID, inst.OS, inst.Arch, string(inst.Provider), strconv.FormatBool(poolManager.IsDistributed()), inst.VariantID, metric.ConvertBool(inst.GPU)).Observe(resp.OSStats.MaxMemUsagePct)
