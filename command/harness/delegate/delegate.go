...
func (c *delegateCommand) run(*kingpin.ParseContext) error {
	...
	if env.Settings.AutoInjectionBinaryURI == "" {
		env.Settings.AutoInjectionBinaryURI = "https://app.harness.io/storage/harness-download/harness-ti/auto-injection/1.0.10"
	}
	c.env = env
	...
	instanceStore, stageOwnerStore, capacityReservationStore, err := database.ProvideStore(c.env.Database.Driver, c.env.Database.Datasource)
	if err != nil {
