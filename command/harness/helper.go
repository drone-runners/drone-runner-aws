...
func getStreamLogger(cfg leapi.LogConfig, mtlsConfig spec.MtlsConfig, logKey, correlationID string) *lelivelog.Writer {
	...
	// Disable local echo/tee to stdout so build log lines do not appear in service logs
	wc := lelivelog.New(client, logKey, correlationID, nil, false, cfg.TrimNewLineSuffix, cfg.SkipOpeningStream)
	go func() {
