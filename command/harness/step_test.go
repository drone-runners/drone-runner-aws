...
// the parent directory on windows). Bundle env vars point at the merged bundle,
// extra env vars at the Harness-only CA, without overriding values the step
// already defines.
func TestConfigureEgressStep(t *testing.T) {
	...
	// Mirrors the config.EgressProxy defaults.
	defaultCfg := config.EgressProxy{
	...
	allCAEnvVars := append(append([]string{}, defaultCfg.CABundleEnvVars...), defaultCfg.CAExtraEnvVars...)

	t.Run("linux with proxy URL sets proxy envs, CA path and file volume", func(t *testing.T) {
		r := &ExecuteVMRequest{}
		configureEgressStep(r, oshelp.OSLinux, proxyURL, noProxy, config.EgressProxy{})
	...
	t.Run("empty proxy URL omits proxy envs but still mounts CA", func(t *testing.T) {
		r := &ExecuteVMRequest{}
		configureEgressStep(r, oshelp.OSLinux, "", noProxy, config.EgressProxy{})
	...
	t.Run("windows mounts parent cert directory", func(t *testing.T) {
		r := &ExecuteVMRequest{}
		configureEgressStep(r, oshelp.OSWindows, proxyURL, noProxy, config.EgressProxy{})
	...
	t.Run("linux injects CA env vars: bundle for replacement-style, CA for additive", func(t *testing.T) {
		r := &ExecuteVMRequest{}
		configureEgressStep(r, oshelp.OSLinux, proxyURL, noProxy, defaultCfg)
	...
	t.Run("windows CA env vars point at the windows CA paths", func(t *testing.T) {
		r := &ExecuteVMRequest{}
		configureEgressStep(r, oshelp.OSWindows, proxyURL, noProxy, defaultCfg)
	...
	t.Run("custom list injects only listed vars and skips empty entries", func(t *testing.T) {
		r := &ExecuteVMRequest{}
		configureEgressStep(r, oshelp.OSLinux, "", noProxy, config.EgressProxy{CABundleEnvVars: []string{"REQUESTS_CA_BUNDLE", ""}})
